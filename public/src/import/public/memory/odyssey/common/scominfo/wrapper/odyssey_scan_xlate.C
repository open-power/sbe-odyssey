...
/*                                                                        */
/* $Source: public/src/import/public/memory/odyssey/common/scominfo/wrapper/odyssey_scan_xlate.C $ */
/*                                                                        */
...
/* IBM_PROLOG_END_TAG                                                     */
///
/// @file odyssey_scan_xlate.C
/// @brief ODYSSEY SCAN address translation test utility
///
...
// DEBUG:
// DEBUG_PRINT is set in internal/common/odyssey/sscsupport/wrapper/odyssey_scan_xlate.mk
...
usage(const std::string& i_prg_name)
{
    ...
    printf("USAGE: %s <target> <address> [-ec <chip EC level>] \n\n", basename(&l_char_array[0]));
    printf("Required parameters:\n");
    printf("  <target> = Odyssey scan target type is chip only\n");
    printf("    Supported values: odyssey/ocmb\n");
    printf("\n");
    printf("  <address> = Chip scan address\n\n");
    printf("Optional parameter:\n");
    printf("  <chip EC level> = Chip EC level, two HEX digits.\n");
    printf("    Example: 20 --> EC level 2.0\n");
...
/// @brief ODYSSEY SCAN address translation utility/test program
...
int main(int i_argc, char** i_argv)
{
    ...
    do
    {
        ...
        printf("(i)       cplt: 0x%02X\n", odyssey_scan_addr::getChiplet(l_addr));
        printf("(i) ext region: 0x%X\n", odyssey_scan_addr::getExtendedRegion(l_addr));
        printf("(i)       vitl: %d\n", odyssey_scan_addr::isVitlRing(l_addr) ? (1) : (0));
        printf("(i)     region: %03X\n", odyssey_scan_addr::getRegion(l_addr));
        printf("(i)  scan type: %X\n", odyssey_scan_addr::getScanType(l_addr));
        ...
        l_rc = odyssey_scaninfo_isChipUnitScanRing(l_addr, l_unit_target_related, l_unit_target_matches);

        if (l_rc)
        {
            printf("ERROR: Error from odyssey_scaninfo_isChipUnitScanRing!\n");
            break;
        }
