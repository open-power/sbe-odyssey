...
/*                                                                        */
/* $Source: public/src/import/public/memory/odyssey/common/scominfo/wrapper/odyssey_scom_xlate.C $ */
/*                                                                        */
...
/* IBM_PROLOG_END_TAG                                                     */
///
/// @file odyssey_scom_xlate.C
/// @brief ODYSSEY SCOM address translation test utility
///
                           ...
                           uint32_t& o_chipEcLevel)
{
    ...
    {
        dbg_print("    i_argv[%d] = %s\n", ii, i_argv[ii]);
    }

    dbg_print("    l_ecArgIdx = %d\n", l_ecArgIdx);

    if (!l_rc)
    {
        dbg_print(" Command line input values\n");
        dbg_print("    i_targetStr   %s\n", o_targetStr.c_str());
        dbg_print("    Chip target   %d\n", o_targetIsChip);
        dbg_print("    i_addr        0x%.8X_0x%.8X\n", (uint32_t)(o_addr >> 32), (uint32_t)(o_addr & 0xFFFFFFFFULL));
        dbg_print("    i_chipUnitNum %d\n", o_chipUnitNum);
        dbg_print("    i_chipEcLevel 0x%.2X\n", o_chipEcLevel);
    }
    else
    {
...
/// @retval None
void usage(const std::string& i_prg_name)
{
    ...
    printf("  <target> = ODYSSEY target type\n");
    printf("    Supported values:\n");
    printf("      Chip: odyssey/ocmb\n");
    printf("      Chip units: odyssey/ocmb.perv, odyssey/ocmb.omi, odyssey/ocmb.mp\n");
...
/// @brief ODYSSEY SCOM address translation utility/test program
...
int main(int i_argc, char** i_argv)
{
    ...
    do
    {
        ...
        odysseyChipUnits_t l_chip_unit_target_type = ODYSSEY_NO_CU;
        odysseyTranslationMode_t l_tx_mode = ODYSSEY_DEFAULT_MODE;
        ...
        else
        {
            ...
            l_rc = odyssey_convertCUString_to_enum(l_chip_unit_str, l_chip_unit_target_type);
            ...
            l_rc = odyssey_validateChipUnitNum(l_unit_target_num, l_chip_unit_target_type);
        ...
        dbg_print("    l_chip_str        %s\n", l_chip_str.c_str());
        dbg_print("    l_chip_unit_str   %s\n", l_chip_unit_str.c_str());
        ...
        odyssey_displayAddrFields(l_addr, l_chip_unit_target_type, l_EcLevel);

        // check for valid address format, find list of any chip unit targets
        bool l_unit_target_related = false;
        std::vector<odyssey_chipUnitPairing_t> l_unit_target_matches;

        l_rc = odyssey_scominfo_isChipUnitScom(l_chip_unit_target_type,
                                               l_EcLevel,
                                               l_addr,
        ...
        if (l_rc)
        {
            printf("ERROR: Error from odyssey_scominfo_isChipUnitScom!\n");
            break;
        }
             ...
             l_iter++)
        {
            ...
            l_rc = odyssey_convertCUEnum_to_String(l_iter->chipUnitType, l_candidate_str);
        ...
        if (!l_target_is_chip)
        {
            ...
            {
                printf("ERROR: Provided chipUnit '%s' doesn't match any chipUnit returned by odyssey_scominfo_isChipUnitScom!\n",
                       l_chip_unit_str.c_str());
            ...
            {
                printf("ERROR: Provided chipUnit '%s' matches more than one chipUnit returned by odyssey_scominfo_isChipUnitScom!\n",
                       l_chip_unit_str.c_str());
                l_rc = E_TYPE_MATCHES_GT1_CANDIDATES;
                break;
            }

            l_addr_abs = odyssey_scominfo_createChipUnitScomAddr(l_chip_unit_target_type,
                         l_EcLevel,
                         l_unit_target_num,
                         l_addr,
                         l_tx_mode);
        ...
        printf("              OUTPUT                   \n");
        printf("-------------------------------------");
        odyssey_displayAddrFields(l_addr_abs, l_chip_unit_target_type, l_EcLevel);
        ...
        // Get clock domain
        CLOCK_DOMAIN l_domain = ODYSSEY_FAKE_DOMAIN;
        l_rc = odyssey_clockcntl_getScomClockDomain(l_chip_unit_target_type,
                l_unit_target_num,
                l_addr_abs,
                l_domain,
                ODYSSEY_DEFAULT_MODE);

        if (l_rc)
        {
            printf("ERROR: odyssey_clockcntl_getScomClockDomain returns an error."
        ...
        else
        {
            ...
            std::string l_domainStr = odyssey_clockcntl_convertClockDomainEnum(l_domain);
            ...
            l_rc = odyssey_clockcntl_clockStateRegisterScreen(l_chip_unit_target_type,
                    l_unit_target_num,
                    l_addr_abs,
                    l_domains,
                    ODYSSEY_DEFAULT_MODE);
            ...
            if (l_rc)
            {
                printf("ERROR: odyssey_clockcntl_clockStateRegisterScreen returns an error."
                       " Address 0x%08lx_%08llx, UnitType %s, Instance %d\n",
                       l_addr_abs >> 32, l_addr_abs & 0xFFFFFFFFULL,
                       l_chip_str.c_str(), l_unit_target_num);
            }
            else if (l_domains.size() != 0)
            {
                ...
                {
                    l_domainStr = odyssey_clockcntl_convertClockDomainEnum(domain);
