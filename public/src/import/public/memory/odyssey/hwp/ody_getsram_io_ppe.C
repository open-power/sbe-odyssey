...
/*                                                                        */
/* $Source: public/src/import/public/memory/odyssey/hwp/ody_getsram_io_ppe.C $ */
/*                                                                        */
...
/* IBM_PROLOG_END_TAG                                                     */
///
/// @file ody_getsram_io_ppe.C
/// @brief Read data from Odyssey IO PPE SRAM
///
...
fapi2::ReturnCode ody_getsram_io_ppe(
    ...
    uint8_t* o_data)
{
    ...
    FAPI_DBG("ody_getsram_io_ppe: i_offset [0x%.8X%.8X], i_bytes %u.",
             ((i_offset >> 32) & 0xFFFFFFFF), (i_offset & 0xFFFFFFFF), i_bytes);

    // Enable auto increment
    if (i_bytes > 8)
    {
        FAPI_DBG("ody_getsram_io_ppe: enable auto-increment.");
        // Enable auto increment
    ...
    // Read data
    FAPI_DBG("Read data from SRAM...");

    while (l_dataPtr < (o_data + i_bytes))
    {
        // Read 8 bytes at a time
        FAPI_TRY(WRAP0_ARB_CSDR.getScom(i_target),
                 "Error getscom WRAP0_ARB_CSDR.");
        l_data = WRAP0_ARB_CSDR;
        ...
        FAPI_DBG("Read data: l_dataPtr %p, l_data64 = 0x%.16llX", l_dataPtr - 1, l_data);
    }
