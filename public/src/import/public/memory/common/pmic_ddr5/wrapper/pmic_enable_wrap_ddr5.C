...
/*                                                                        */
/* $Source: public/src/import/public/memory/common/pmic_ddr5/wrapper/pmic_enable_wrap_ddr5.C $ */
/*                                                                        */
...
///
/// @file pmic_enable_wrap_ddr5.C
/// @brief Wrapper for calling pmic_enable_ddr5
///
...
static const std::string PROCEDURE = "pmic_enable_wrap_ddr5";
void help()
{
    // procedure constants
    ...
    snprintf(outstr, sizeof(outstr), "Git commit-id %s \n",
             GIT_COMMIT_ID.c_str());
    ecmdOutput(outstr);
    ecmdOutput("        [-m [mode]] [-h] [-k#] [-n#] [-s#] [-p#] [-d] [-quiet] [-verif]\n");
    ecmdOutput("\n");
    ecmdOutput("Additional options:\n");
    ecmdOutput("      -h                           Display this help message.\n");

    ecmdOutput("      -m [mode]                    Enables PMIC with settings other than those defined in vendor region.\n");
    ecmdOutput("                                   mode values:\n");
    ecmdOutput("                                     spd     :  Start PMIC with fields derived from SPD (default)\n");
    ecmdOutput("                                     manual  :  Start PMIC with current settings (last/biased settings)\n");
...
int main(int argc, char* argv[])
{
    ...
    char* argdata;
    mss::pmic::enable_mode mode = mss::pmic::enable_mode::SPD;
    char outstr[256];
    ...
    do
    {
        ...
        if (argdata != NULL)
        {
            ...
            if (mstr == "spd")
            {
                ...
                mode = mss::pmic::enable_mode::SPD;
            }
            else if (mstr == "manual")
            {
                mode = mss::pmic::enable_mode::MANUAL;
            }
            else
            {
                ecmdOutputError("Unknown enable mode! [spd/manual]\n");
                break;
            }
        }
        ...
        while (ecmdConfigLooperNext(node_target, node_looper))
        {
            ...
            l_ocmb_target.chipType = "odyssey";
            ...
            while(ecmdConfigLooperNext(l_ocmb_target, ocmb_looper))
            {
                ...
                rc_fapi = mss::pmic::ddr5::pmic_enable(fapi_target, mode);
