...
/*                                                                        */
/* $Source: public/src/import/public/memory/common/pmic/lab/tools/pmic_update_tool.C $ */
/*                                                                        */
...
/* IBM_PROLOG_END_TAG                                                     */
///
/// @file pmic_update_tool.C
/// @brief Update PMIC device vendor region with given parameters
///
...
int main(int i_argc, char* i_argv[])
{
    ...
    printf("Git commit ID: %s Procedure: pmic_update_tool.exe \n",
           GIT_COMMIT_ID.c_str());

    // Struct for user input
    mss::lab::pmic::update::inputs l_inputs;

    rc = mss::lab::pmic::update::process_inputs(i_argc, i_argv, l_inputs);
    ...
    for (const auto& ecmd_ocmb_target : ecmd_target_list)
    {
        ...
        // Run procedure
        rc = mss::lab::pmic::update_chip(fapi2_ocmb_target, l_inputs.iv_id, l_inputs.iv_setting, l_inputs.iv_value);
