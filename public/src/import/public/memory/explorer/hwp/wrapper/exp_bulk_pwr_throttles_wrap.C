...
/*                                                                        */
/* $Source: public/src/import/public/memory/explorer/hwp/wrapper/exp_bulk_pwr_throttles_wrap.C $ */
/*                                                                        */
...
/// @file exp_bulk_pwr_throttles_wrap.C
/// @brief Wrapper for calling exp_bulk_pwr_throttles hwp
///
...
static const std::string PROCEDURE = "exp_bulk_pwr_throttles_wrap";
void help()
{
    // procedure constants
...
int main(int argc, char* argv[])
{
    ...
    do
    {
        ...
        while (ecmdConfigLooperNext(node_target, node_looper))
        {
            ...
            std::vector< ecmdChipTarget > ecmd_targets;
            ...
            {
                // collect ocmb target
                // Populate list of ecmd targets
                ecmd_targets.push_back(l_ocmb_target);
            }

            std::vector< fapi2::Target<fapi2::TARGET_TYPE_OCMB_CHIP> > fapi_targets;

            for(const auto& target : ecmd_targets)
            {
                // Setting up a fapi2::Target from an ecmdChipTarget and adding to vector
                fapi_targets.push_back ( fapi2::plat_target_handle_t(&target) );
            }
