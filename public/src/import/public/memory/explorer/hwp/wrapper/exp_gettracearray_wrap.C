...
/*                                                                        */
/* $Source: public/src/import/public/memory/explorer/hwp/wrapper/exp_gettracearray_wrap.C $ */
/*                                                                        */
...
#define procedureName "exp_gettracearray"
#define REVISION "1.0"
...
{
    std::string name;
    exp_tracearray_id trace_array;
};

static const bus_def bus_defs[] =
{
    {"TRACE0", EXP_TA_TRACE0},
    {"TRACE1", EXP_TA_TRACE1},
};
...
int main(int argc, char* argv[])
{
    ...
    ecmdDllInfo DLLINFO;
    exp_gettracearray_args gettracearray_args;
    ...
    while (ecmdConfigLooperNext(target, looper))
    {
        ...
        fapi2::variable_buffer array_data(EXP_TRACEARRAY_NUM_ROWS * EXP_TRACEARRAY_BITS_PER_ROW);
        FAPI_EXEC_HWP(rc_fapi, exp_gettracearray, fapi_target, gettracearray_args, array_data);
        rc = (uint64_t)rc_fapi;
        ...
        {
            for (unsigned int i = 0; i < EXP_TRACEARRAY_NUM_ROWS; i++)
            {
                char str[256];
