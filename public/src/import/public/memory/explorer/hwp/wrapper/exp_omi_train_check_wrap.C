...
/*                                                                        */
/* $Source: public/src/import/public/memory/explorer/hwp/wrapper/exp_omi_train_check_wrap.C $ */
/*                                                                        */
...
/// @file exp_omi_train_check_wrap.C
/// @brief Wrapper for calling exp_omi_train_check hwp
///
...
static const std::string PROCEDURE = "exp_omi_train_check_wrap";
void help()
{
    // procedure constants
...
int main(int argc, char* argv[])
{
    ...
    do
    {
        ...
        while (ecmdConfigLooperNext(node_target, node_looper))
        {
            ...
            // Keep running loops, but hold onto the bad rc
            if (rc)
            {
                bad_rc = rc;
            }
    ...
    // Replace with the bad rc if we had one from a run while looping through the targets
    if (bad_rc)
    {
        rc = bad_rc;
    }
