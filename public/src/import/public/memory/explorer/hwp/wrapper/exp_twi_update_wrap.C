...
/*                                                                        */
/* $Source: public/src/import/public/memory/explorer/hwp/wrapper/exp_twi_update_wrap.C $ */
/*                                                                        */
...
/// @brief Wrapper for updating the explorer fw through the twi interface
///
...
static const std::string PROCEDURE = "exp_twi_update_wrap";
void help()
{
    // procedure constants
    ...
    ecmdOutput(outstr);
    ecmdOutput("        [-h] [-k#] [-n#] [-s#] [-p#] [-d] [-quiet] [-verif] image_file\n");
    ecmdOutput("Parameters:\n");
    ecmdOutput("      image_file      The exploer flash loader binary file\n");
    ecmdOutput("\n");
    ecmdOutput("Options:\n");
    ...
    do
    {
        ...
        while (ecmdConfigLooperNext(node_target, node_looper))
        {
            ...
            while(ecmdConfigLooperNext(l_ocmb_target, ocmb_looper))
            {
                ...
                // Get Explorer into TWI update mode
                fapi2::Target<fapi2::TARGET_TYPE_OCMB_CHIP> fapi_target(&l_ocmb_target);
                rc = static_cast<uint32_t>(exp_twi_enable(fapi_target));

                if (rc)
                {
                    snprintf(outstr, sizeof(outstr), "ERROR: %s FAPI call for exp_twi_enable exited with bad return code = %s 0x%08x\n",
                             PROCEDURE.c_str(),
                ...
                // Now download the image to Explorer
                rc = static_cast<uint32_t>(exp_twi_update(fapi_target, l_image_data, l_image_size));

                if (rc)
                {
                    snprintf(outstr, sizeof(outstr), "ERROR: %s FAPI call for exp_twi_update exited with bad return code = %s 0x%08x\n",
                             PROCEDURE.c_str(),
                             ecmdParseReturnCode(rc).c_str(), rc);
                    ecmdOutputError(outstr);
                    break;
