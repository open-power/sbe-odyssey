...
/*                                                                        */
/* $Source: public/src/import/public/memory/explorer/hwp/exp_sbe_tracearray.C $ */
/*                                                                        */
...
const uint32_t TRACE1_SCOM_BASE = 0x08010440;
...
// HWP entry point
//------------------------------------------------------------------------------
extern "C" fapi2::ReturnCode exp_sbe_tracearray(
    ...
    const uint16_t i_num_rows)
{
    ...
    const uint32_t TRACE_SCOM_BASE = (i_args.trace_array == EXP_TA_TRACE0) ? TRACE0_SCOM_BASE : TRACE1_SCOM_BASE;
    const bool first_granule = (0 == i_first_row);
    const bool last_granule  = ((i_first_row + i_num_rows) >= EXP_TRACEARRAY_NUM_ROWS);

    // mark HWP entry
    FAPI_INF("Entering ...");

    FAPI_ASSERT(i_args.trace_array < _EXP_TA_COUNT,
                fapi2::EXP_GETTRACEARRAY_INVALID_ARRAY()
    ...
    if (i_args.collect_dump && first_granule)
    {
        ...
        FAPI_ASSERT(l_trace_ctrl.getBit<TRCTRL_CLOCK_ENABLE>(),
                    fapi2::EXP_GETTRACEARRAY_CLOCKS_OFF()
                    .set_TARGET(i_target).set_TRACE_ARRAY(i_args.trace_array)
        ...
        FAPI_ASSERT(!l_trace_ctrl.getBit<TRCTRL_RUN_STATUS>(),
                    fapi2::EXP_GETTRACEARRAY_TRACE_RUNNING()
                    .set_TARGET(i_target).set_TRACE_ARRAY(i_args.trace_array)
                    .set_TRCTRL_REG(l_trace_ctrl),
    ...
    if (i_args.collect_dump)
    {
        ...
        for (int i = 0; i < EXP_TRACEARRAY_NUM_ROWS; i++)
        {
