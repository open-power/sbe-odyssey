...
/*                                                                        */
/* $Source: public/src/import/public/hwp/generic/perv/poz_sbe_hreset.C $  */
/*                                                                        */
...
#include <poz_sbe_hreset_regs.H>

using namespace fapi2;

ReturnCode poz_sbe_hreset(
    const Target<TARGET_TYPE_ANY_POZ_CHIP>& i_target,
    const poz_sbe_boot_parms i_boot_parms, bool i_use_cfam_path)
{
    FAPI_DBG("Entering ...");
    ...
    if (i_use_cfam_path)
    {
        ...
        {
            FAPI_DBG("Skipping HRESET, SBE is NOT in Runtime state");
            goto fapi_try_exit;
        }
    }
    else
