...
/*                                                                        */
/* $Source: public/src/import/public/emb/pst/pmlib/include/pmheaders/hcode_errl_table.h $ */
/*                                                                        */
...
#define HCODE_ELOG_TABLE_MAGIC_NUMBER 0x454C54 ///< "ELT"
...
typedef struct
{
    union
    {
        ...
        struct
        {
            uint64_t errlog_id              : 8;    ///< Error log id
            uint64_t errlog_src             : 8;    ///< Error log source
            uint64_t errlog_len             : 16;   ///< Error log length
            uint64_t errlog_addr            : 32;   ///< Error log address
        } fields;
    } dw0;  ///< Doubleword 0
} hcode_elog_entry_t;
...
typedef struct hcode_error_table
{
    union
    {
        ...
        struct
        {
            ...
            uint64_t version                : 8;    ///< Log version
            uint64_t total_log_slots        : 8;    ///< Total number of log slots
            uint64_t log_address_valid      : 8;    ///< Log address valid flag
