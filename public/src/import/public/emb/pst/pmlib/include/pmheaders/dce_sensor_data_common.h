...
/*                                                                        */
/* $Source: public/src/import/public/emb/pst/pmlib/include/pmheaders/dce_sensor_data_common.h $ */
/*                                                                        */
...
typedef union
{
    ...
    struct
    {
        uint16_t  reading[4];       ///< Readings per region
    } half_words;                   ///< halfword fields
} DtsSensorReg_t;

///
/// @brief DTS Sensor Reading
///
typedef union
{
    uint16_t reading;               ///< Raw reading
    struct
    {
        uint16_t result   : 12;     ///< Result field
        uint16_t id       :  2;     ///< Id field
        uint16_t overflow :  1;     ///< Overflow field
        uint16_t valid    :  1;     ///< Valid field
    } fields;                       ///< sub fields
} DtsSensorReading_t;
...
typedef struct
{
    ...
    DtsSensorReading_t mma;         ///< MMA
    DtsSensorReading_t cache;       ///< Cache
} CoreDataDts_t;
...
typedef struct
{
    DtsSensorReading_t dragstrip;   ///< Dragstrip reading
} QuadDataDts_t;

///
/// @brief EMPATH Core activity counts
///
typedef struct
{
...
///
/// @brief EMPATH Core Throttle counts
///
typedef struct
{
    ...
    uint32_t harsh_throttle;        ///< 0x260  Harsh Throttle blocked
} CoreDataThrottle_t;

///
/// @brief EMPATH Core Droops counts
///
typedef struct
{
...
typedef union
{
    ...
    struct
    {
        uint64_t data : 5;          ///< Data field
        uint64_t data_min : 5;      ///< Min field
        uint64_t data_max : 5;      ///< Max field
        uint64_t not_used : 49;     ///< Unused field
    } fields;                       ///< sub fields
} CpmsSdsr_t;
...
typedef union
{
    ...
    struct
    {
        uint64_t dds_valid : 1;     ///< Valid field
        uint64_t dds_reading : 15;  ///< Valid field
        uint64_t dds_min_valid : 1; ///< Min valid field
        uint64_t dds_min : 15;      ///< Min field
        uint64_t dds_max_valid : 1; ///< Max valid field
        uint64_t dds_max : 15;      ///< Max field
        uint64_t not_used : 16;     ///< Unused field
    } fields;                       ///< sub fields
} DdsData_t;
...
///
/// @brief Core level data struct containing Empath counts and DTS reads
///
/// Notes:
...
///
/// @brief level data struct containing DTS reads
///
/// Note:  See previous notes to CoreData_t.
///
typedef struct
{
    QuadDataDts_t              dts;             ///< Quad DTS
} QuadData_t;


///
/// @brief High-level DCE sensor data struct for DCE, OCE and 405
///
/// Notes:
...
///
/// @brief Enum of DCE status flags (used by DCESensorData_t.dce_status_flag)
///
...
///
/// @brief Enum of DCE error coded (used by DCESensorData_t.dce_error_code)
///
/// Notes:
...
///
/// @brief Variable used by DCE and OCE to signal eachother about their usage of the DCESensorData data.
...
///
/// @brief Enum indicating the DCE-OCE usage status of the most recently collected DCESensorData data set.
///
/// @note
...
{
    DCE_DATA_USAGE_STATUS_UNDEFINED = 0xff, ///< Undefined
