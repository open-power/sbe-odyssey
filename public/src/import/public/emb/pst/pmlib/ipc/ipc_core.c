...
/*                                                                        */
/* $Source: public/src/import/public/emb/pst/pmlib/ipc/ipc_core.c $       */
/*                                                                        */
...
#include "kernel.h"
#include "ipc_api.h"
#include "pmhw_shared_data.h"
#if __PPE42__
    #include "ppe42_cache.h"
#endif

#if defined(__OCC_PLAT)
const uint8_t G_ipi_irq[PLATHW_MAX_INSTANCES] =
{
...
int ipc_send_msg(ipc_msg_t* msg, uint32_t target_id)
{
    ...
    do
    {
        ...
        if(target_id > PMHW_INST_ID_MAX)
        {
            rc = IPC_RC_INVALID_TARGET_ID;
            break;
        }

        target_cbufs = &PLATSD_PTR->ipc_data.targets[target_id];
        msgs = &target_cbufs->cbufs[PMHW_INST_ID_SELF][0];
        read_count = &target_cbufs->counts.reads.counts8[PMHW_INST_ID_SELF];
        write_count = &target_cbufs->counts.writes.counts8[PMHW_INST_ID_SELF];
...
int ipc_send_cmd(ipc_msg_t* cmd)
{
    ...
    do
    {
        ...
        cmd->func_id.sender_id = PMHW_INST_ID_SELF;
...
{
    ipc_target_t*            my_cbufs = &PLATSD_PTR->ipc_data.targets[PMHW_INST_ID_SELF];
...
#endif
{
    ...
    KERN_IRQ_DISABLE(IPC_GET_IRQ(PMHW_INST_ID_SELF));
    ...
    KERN_IRQ_STATUS_CLEAR(IPC_GET_IRQ(PMHW_INST_ID_SELF));

    my_cbufs = &PLATSD_PTR->ipc_data.targets[PMHW_INST_ID_SELF];
    ...
    KERN_IRQ_ENABLE(IPC_GET_IRQ(PMHW_INST_ID_SELF));
#endif
}
...
int ipc_enable(void)
{
    ...
    do
    {
        ...
        rc = KERN_IRQ_HANDLER_SET(IPC_GET_IRQ(PMHW_INST_ID_SELF),
                                  ipc_irq_handler,
                                  0,
                                  KERN_CRITICAL);
...
int ipc_disable(uint32_t target_id)
{
    ...
    do
    {
        ...
        if(target_id == PMHW_INST_ID_SELF)
        {
            G_ipc_enabled = 0;
        }
