...
/*                                                                        */
/* $Source: public/src/import/public/emb/odyssey/io/ioo/tx_seg_test.c $   */
/*                                                                        */
...
//-     set_tx_dcc_debug_tx_seg_test(0xC38B, tx_seg_test_1r_segs_l    );
    ...
    while(tx_seg_test_next_sr(&sr, tx_seg_test_frc_2r_l, tx_seg_test_2r_seg_l, tx_seg_test_1r_segs_l))
    {
        ...
        status |= tx_seg_test_test_seg(gcr_addr_i);
...
{
    set_debug_state(0xC310, 3); // tx_seg_test_setup begin
...
{
    set_debug_state(0xC320, 3); // tx_seg_test_next_sr begin
...
//-             set_tx_dcc_debug_tx_seg_test(0xC3A9, num_1r_equiv_i); // fix_2r_true, value arbitrary, for code tracing only
            ...
            if (updated)
            {
//-                 set_tx_dcc_debug/_tx_seg_test(0xC3AA, num_1r_equiv_i); // updated, value arbitrary, for code tracing only
                if (num_2r_i > 0)   // need to shift 2r enable
                {
                    if (*sr_io & (sr_post_main_1r_mask | sr_pre1_main_1r_mask |
                                  sr_pre2_main_1r_mask)) // keep 2r segs in pre1 main, pre2 main, post main, or main
                    {
//-                         set_tx_dcc_debug_tx_seg_test(0xC3AB, num_1r_equiv_i); // shift_2r leave_in_main, value arbitrary, for code tracing only
                        ...
                        {
                            shift_mask_up_to_end(sr_io, sr_post_main_2r_mask | sr_pre1_main_2r_mask | sr_pre2_main_2r_mask | sr_main_2r_mask);
                        }
                    }
...
//-                         set_tx_dcc_debug_tx_seg_test(0xC3AC, num_1r_equiv_i); // shift_2r from_main_to_ffe, value arbitrary, for code tracing only
                        // The post 2r segment control reg is sinkless when the RLM connects to a 7nm TX.
                        // To handle this, if PPE is in an Odyssey unit (7nm), skip over the 2r post seg
                        // and start with the next 2r seg in the SR, which is pre1.
                        if ( is_odyssey() )
                        {
                        }
                        else
                        {
                            *sr_io = (*sr_io & ~(sr_post_main_2r_mask | sr_pre1_main_2r_mask | sr_pre2_main_2r_mask | sr_main_2r_mask)) |
                                     sr_post_ffe_2r_mask;
                        }
                    }
                    else if (*sr_io & (sr_post_ffe_2r_mask | sr_pre1_ffe_2r_mask)) // keep 2r segs in pre1 ffe, pre2 ffe, or post ffe
                    {
                        shift_mask_up_to_end(sr_io, sr_post_ffe_2r_mask | sr_pre1_ffe_2r_mask | sr_pre2_ffe_2r_mask);
                    }

                    //else { // the pre2 2r is the last in the sr, so stay there
//-                         set_tx_dcc_debug_tx_seg_test(0xC3AD, num_1r_equiv_i); // shift_2r to_end_of_main, value arbitrary, for code tracing only
                    //    *sr_io = (*sr_io|sr_pre2_ffe_2r_mask);
                    //}
                } // num_2r_i > 0
...
{
    set_debug_state(0xC330, 3); // tx_seg_test_sr_to_reg begin

    if ((sr_pre1_main_2r_mask & sr_i) | (sr_pre1_main_1r_mask & sr_i)) // pre1 main enabled
    {
        update_reg(seg_regs_io->pre1_reg, tx_nseg_pre1_en_endbit - sr_pre1_main_1r_width,    sr_pre1_main_2r_width,  sr_i,
                   sr_pre1_main_2r_endbit );
        update_reg(seg_regs_io->pre1_reg, tx_nseg_pre1_en_endbit,                            sr_pre1_main_1r_width,  sr_i,
                   sr_pre1_main_1r_endbit );
        seg_regs_io->pre1_reg |= tx_pre2_sel_mask; // turn on selects to select main
    }
    else
    {
        update_reg(seg_regs_io->pre1_reg, tx_nseg_pre1_en_endbit - sr_pre1_ffe_1r_width,     sr_pre1_ffe_2r_width,  sr_i,
                   sr_pre1_ffe_2r_endbit  );
        update_reg(seg_regs_io->pre1_reg, tx_nseg_pre1_en_endbit,                            sr_pre1_ffe_1r_width,  sr_i,
                   sr_pre1_ffe_1r_endbit  );
        seg_regs_io->pre1_reg &= ~tx_pre2_sel_mask; // turn off selects to select ffe
    }

    if ((sr_pre2_main_2r_mask & sr_i) | (sr_pre2_main_1r_mask & sr_i)) // pre2 main enabled
    {
        update_reg(seg_regs_io->pre2_reg, tx_nseg_pre2_en_endbit - sr_pre2_main_1r_width,    sr_pre2_main_2r_width,  sr_i,
                   sr_pre2_main_2r_endbit );
        update_reg(seg_regs_io->pre2_reg, tx_nseg_pre2_en_endbit,                            sr_pre2_main_1r_width,  sr_i,
                   sr_pre2_main_1r_endbit );
        seg_regs_io->pre2_reg |=
            tx_pre2_sel_mask; // turn on selects to select main
    }
    else
    {
        update_reg(seg_regs_io->pre2_reg, tx_nseg_pre2_en_endbit - sr_pre2_ffe_1r_width,     sr_pre2_ffe_2r_width, sr_i,
                   sr_pre2_ffe_2r_endbit  );
        update_reg(seg_regs_io->pre2_reg, tx_nseg_pre2_en_endbit,                            sr_pre2_ffe_1r_width, sr_i,
                   sr_pre2_ffe_1r_endbit  );
        seg_regs_io->pre2_reg &=
            ~tx_pre2_sel_mask; // turn off selects to select ffe
    }

    if ((sr_post_main_2r_mask & sr_i) | (sr_post_main_1r_mask & sr_i)) // post main enabled
    {
        update_reg(seg_regs_io->post_reg, tx_nseg_post_en_endbit - sr_post_main_1r_width,    sr_post_main_2r_width,  sr_i,
                   sr_post_main_2r_endbit );
        update_reg(seg_regs_io->post_reg, tx_nseg_post_en_endbit,                            sr_post_main_1r_width,  sr_i,
                   sr_post_main_1r_endbit );
        seg_regs_io->post_reg |=
            tx_post_sel_mask; // turn on selects to select main
    }
    else
    {
        update_reg(seg_regs_io->post_reg, tx_nseg_post_en_endbit - sr_post_ffe_1r_width,     sr_post_ffe_2r_width, sr_i,
                   sr_post_ffe_2r_endbit  );
        update_reg(seg_regs_io->post_reg, tx_nseg_post_en_endbit,                            sr_post_ffe_1r_width, sr_i,
                   sr_post_ffe_1r_endbit  );
        seg_regs_io->post_reg &=
            ~tx_post_sel_mask; // turn off selects to select ffe
    }

    update_reg(seg_regs_io->main, tx_nseg_main_en_endbit - sr_main_1r_width, sr_main_2r_width, sr_i, sr_main_2r_endbit );
    update_reg(seg_regs_io->main, tx_nseg_main_en_endbit,                    sr_main_1r_width, sr_i, sr_main_1r_endbit );
} //tx_seg_test_sr_to_reg
...
void tx_seg_test_update_seg_gcr( t_gcr_addr* gcr_addr_i, t_tx_seg_regs* prev_seg_regs_i,  t_tx_seg_regs* new_seg_regs_i,
                                 bool update_all_i)
{
    set_debug_state(0xC340, 3); // tx_seg_test_update_seg_gcr begin

    if ((prev_seg_regs_i->pre1_reg ^ new_seg_regs_i->pre1_reg) || update_all_i)
    {
        if (update_all_i || (~(prev_seg_regs_i->pre1_reg ^ prev_seg_regs_i->pre1_reg)&tx_pre1_sel_mask))
        {
            put_ptr_field_fast(gcr_addr_i, tx_pseg_pre1_en, 0x0);
            put_ptr_field_fast(gcr_addr_i, tx_pre1_sel,     0x0);
        }

        put_ptr_field_fast(gcr_addr_i, tx_nseg_pre1_en, new_seg_regs_i->pre1_reg);
        put_ptr_field_fast(gcr_addr_i, tx_pseg_pre1_en, new_seg_regs_i->pre1_reg);
        put_ptr_field_fast(gcr_addr_i, tx_pre1_sel,     new_seg_regs_i->pre1_reg);
    }

    if ((prev_seg_regs_i->pre2_reg ^ new_seg_regs_i->pre2_reg) || update_all_i)
    {
        if (update_all_i || (~(prev_seg_regs_i->pre2_reg ^ prev_seg_regs_i->pre2_reg)&tx_pre2_sel_mask))
        {
            put_ptr_field_fast(gcr_addr_i, tx_pseg_pre2_en, 0x0);
            put_ptr_field_fast(gcr_addr_i, tx_pre2_sel,     0x0);
        }

        put_ptr_field_fast(gcr_addr_i, tx_nseg_pre2_en, new_seg_regs_i->pre2_reg);
        put_ptr_field_fast(gcr_addr_i, tx_pseg_pre2_en, new_seg_regs_i->pre2_reg);
        put_ptr_field_fast(gcr_addr_i, tx_pre2_sel,     new_seg_regs_i->pre2_reg);
    }

    if ((prev_seg_regs_i->post_reg ^ new_seg_regs_i->post_reg) || update_all_i)
    {
        if (update_all_i || (~(prev_seg_regs_i->post_reg ^ prev_seg_regs_i->post_reg)&tx_post_sel_mask))
        {
            put_ptr_field_fast(gcr_addr_i, tx_pseg_post_en, 0x0);
            put_ptr_field_fast(gcr_addr_i, tx_post_sel,     0x0);
        }

        put_ptr_field_fast(gcr_addr_i, tx_nseg_post_en, new_seg_regs_i->post_reg);
        put_ptr_field_fast(gcr_addr_i, tx_pseg_post_en, new_seg_regs_i->post_reg);
        put_ptr_field_fast(gcr_addr_i, tx_post_sel,     new_seg_regs_i->post_reg);
    }

    if ((prev_seg_regs_i->main ^ new_seg_regs_i->main) || update_all_i)
    {
        put_ptr_field_fast(gcr_addr_i, tx_nseg_main_en, new_seg_regs_i->main);
        put_ptr_field_fast(gcr_addr_i, tx_pseg_main_en, new_seg_regs_i->main);
    }
} // tx_seg_test_update_seg_gcr
...
{
    set_debug_state(0xC350, 3); // tx_seg_test_test_seg begin
    int status = rc_no_error;

    uint32_t tst_rcv_p_n_l = 0;
...
//-         set_tx_dcc_debug_tx_seg_test(0xC3E1, tst_rcv_p_n_l);
        ...
        set_debug_state(0xC351, 3); // tx_seg_test_test_seg fail testing for 1
...
//-         set_tx_dcc_debug_tx_seg_test(0xC3E2, tst_rcv_p_n_l);
        ...
        // keep these 2 lines together to follow fir setting idiom
        set_debug_state(0xC352, 3); // tx_seg_test_test_seg fail testing for 0
        set_fir(fir_code_dft_error | fir_code_bad_lane_warning); // set DFT FIR bit
        status = error_code;
    }
...
{
    set_debug_state(0xC360, 3); // tx_seg_test_restore begin
