...
/*                                                                        */
/* $Source: public/src/import/public/emb/odyssey/io/ioo/eo_rxbist_ber.c $ */
/*                                                                        */
...
int eo_rxbist_ber(t_gcr_addr* gcr_addr, const uint32_t lane_mask, const t_bank bank)
{
    ...
    FOR_LESS(lane, l_num_lanes)
    {
        ...
        put_ptr_field(gcr_addr, rx_berpl_cnt_en_exp_sel_alias, 0b111010, read_modify_write); //pl
    ...
    FOR_LESS(num_of_bumps, 4)
    {
        ...
        status |= min_pr_shift( gcr_addr, mini_pr_pos, mini_pr_pos_next, lane_mask, bank );
        // update the position
        mini_pr_pos = mini_pr_pos_next;

        set_debug_state(0x5117, 3);
