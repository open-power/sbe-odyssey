...
/*                                                                        */
/* $Source: public/src/import/public/emb/odyssey/io/common/supervisor_thread.c $ */
/*                                                                        */
...
// vbr21082400 |vbr     | Move checking of thread active time to completely in io_sleep.
// vbr21071900 |vbr     | Relaxed thread active time limit (15us -> 24us) for IOT only
// vbr21060200 |vbr     | Relaxed thread active time limit (12us -> 15us)
// vbr21052400 |vbr     | Use the new thread_active_time_exceeded fir bit
// vbr21051300 |vbr     | Added checking of interval between io_sleep calls (thread active time)
    ...
    while (true)
    {
        ...
        else     //!fw_stop_thread
        {
            ...
            if (current_time > recal_not_run_check_time)
            {
                ...
                for (thread = 0; thread < io_threads; thread++)
                {
                    // Skip this check if disabled for the thread
                    int recal_not_run_disable = mem_regs_u16_base_get(pg_base_addr(ppe_recal_not_run_disable_addr, thread),
                                                ppe_recal_not_run_disable_mask, ppe_recal_not_run_disable_shift);

                    if (recal_not_run_disable)
                    {
                        continue;
                    }
