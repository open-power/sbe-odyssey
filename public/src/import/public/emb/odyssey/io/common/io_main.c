...
/*                                                                        */
/* $Source: public/src/import/public/emb/odyssey/io/common/io_main.c $    */
/*                                                                        */
...
// *! FILENAME    : io_main.c
// *! TITLE       :
// *! DESCRIPTION : IO Main Code - first user code called by kernel.
// *!
...
// vbr21051300 |vbr     | Initialization of variable for checking interval between io_sleep calls.
...
#if defined(IOO) && !defined(UCT)
    #include "ioo_thread.h"
#endif
#if defined(IOT) && !defined(UCT)
...
// Constants for thread_id.  Have up to C_MAX_THREADS threads.
int thread_id[C_MAX_THREADS] = { 0, 1, 2, 3, 4, 5, 6, 7, 8 };
PK_STATIC_ASSERT(C_MAX_THREADS == 9);

// Thread handles
PkThread G_io_thread[C_MAX_THREADS];
...
int main(int argc, char** argv)
{
    ...
    // Initialize the round robin loop counter
    round_robin_loop_count = 0;
    ...
    final_io_thread = io_threads - 1; // The IO threads have IDs [0, io_threads-1], the Supervisor thread has ID io_threads

#if IO_THREADING_METHOD == 2 // Round Robin With Servo Pending
    // Enable only the servo interrupts for the enabled threads
    uint32_t intr_and_mask_val = (1 << scom_ppe_intr_and_mask_width) - 1;
    intr_and_mask_val ^= (intr_and_mask_val >> io_threads);
    lcl_put(scom_ppe_intr_and_mask_lcl_addr, scom_ppe_intr_and_mask_width, intr_and_mask_val);
#endif
    ...
    // Simple Round Robin: Create Shared Thread Barrier Semaphore
    ...
    for (thread = 0; thread < io_threads; thread++)
    {
        ...
        // Round Robin with Servo Pending: Create Individual Thread Barrier Semaphore
        pk_semaphore_create(&thread_barrier[thread], 0, 0);
#endif
    ...
    // Initialize the start time for thread active checking
    last_io_sleep_call_time = pk_timebase_get();
