...
/*                                                                        */
/* $Source: public/src/import/public/emb/p11/kernels/ppe/iota/test/iota_main1.C $ */
/*                                                                        */
...
uint8_t fake_hw_image[1024];
uint8_t unpacked[1024];
...
int main()
{
    ...
    FileArchive  arc(fake_hw_image);
    FileArchive::Entry entry;

    int rc = arc.locate_file("Some_Binary",entry);
    if(rc)
    {
        APPCFG_TRACE("Could not find binary in hw image");
    }
    else
    {
        if(entry.get_size() <= sizeof(unpacked))
        {
            entry.decompress(unpacked, entry.get_size(), NULL);
        }
    }
