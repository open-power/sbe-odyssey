...
/*                                                                        */
/* $Source: public/src/import/public/common/libs/crypto/ec_verify.C $     */
/*                                                                        */
...
// static const bn_t ec_order_qn[ NWORDS ] =
    {
        ...
        0xae79787c40d06994ULL,
        0x8033feb708f65a2fULL,
        0xc44a36477663b851ULL,
        0x449048e16ec79bf6ULL,
    },
    {
        0x00000000ULL,
...
static int bn_cmp (const bn_t* a, const bn_t* b)
{
    ...
    int res = memcmp(a, b, sizeof(bn_t) * NWORDS);
...
int ecdh_derive_key(const unsigned char* publicpt, const unsigned char* privatept, unsigned char* derivedpt)
{
    bn_t public_x[ NWORDS ], public_y[ NWORDS ], private_key[ NWORDS ], temp[ NWORDS ];
    bn_read_pt(public_x, publicpt);
    bn_read_pt(public_y, publicpt + EC_COORDBYTES);
    bn_read_pt(private_key, privatept);

    if (!on_curve(publicpt))
    {
        return 0;
    }

    ec_multiply(public_x, public_y, temp, private_key);
    ec_projective2affine(public_x, temp);
    ...
    for (uint32_t i = 0; i < NWORDS; i++)
    {
        for (uint32_t j = 0; j < sizeof(bn_t); j++)
        {
            ...
            {
                derivedpt[i * sizeof(bn_t) + j - (BNBYTES - EC_PRIMEBYTES)] = (public_x[i] >> (8 * (sizeof(bn_t) - 1 - j))) & 0xFF;
            }
        }
    }
...
int ec_derive_public_key(const unsigned char* privatept, unsigned char* publicxpt, unsigned char* publicypt)
{
    bn_t px[ NWORDS ], py[ NWORDS ], pz[ NWORDS ], private_key[ NWORDS ];
    BN_COPY(px, consts_p()->prime_px);
    BN_COPY(py, consts_p()->prime_py);
    bn_read_pt(private_key, privatept);
    ec_multiply(px, py, pz, private_key);
    ec_projective2affine(px, pz);
    ec_projective2affine(py, pz);
    ...
    for (uint32_t i = 0; i < NWORDS; i++)
    {
        for (uint32_t j = 0; j < sizeof(bn_t); j++)
        {
            if (i * sizeof(bn_t) + j >= (BNBYTES - EC_PRIMEBYTES))
            {
                publicxpt[i * sizeof(bn_t) + j - (BNBYTES - EC_PRIMEBYTES)] = (px[i] >> (8 * (sizeof(bn_t) - 1 - j))) & 0xFF;
                publicypt[i * sizeof(bn_t) + j - (BNBYTES - EC_PRIMEBYTES)] = (py[i] >> (8 * (sizeof(bn_t) - 1 - j))) & 0xFF;
            }
        }
    }
...
int ecdsa_sign(const unsigned char* privatept,   /*   EC_COORDBYTES */
               const unsigned char* hash,        /*   EC_HASHBYTES  */
               unsigned char* signature,         /* 2*EC_COORDBYTES */
               const unsigned char* randnumpt)   /*   EC_COORDBYTES */
{
    bn_t private_key[ NWORDS ], z[ NWORDS ], random[ NWORDS ], randominv[ NWORDS ], prx[ NWORDS ], pry[ NWORDS ],
         temp[ NWORDS ];

    bn_read_pt(private_key, privatept);
    bn_read_hash(z,  hash);
    bn_read_pt(random, randnumpt);
    ...
    BN_COPY(prx, consts_p()->prime_px);            // (prx,pry) <- (base point)
    BN_COPY(pry, consts_p()->prime_py);
    ec_multiply(prx, pry, temp, random);
    ec_projective2affine(prx, temp);

    if (bn_ge_order(prx) ||  bn_is_zero(prx, 0))
    {
        printf("Point is zero!!!!");
    }

    bn_modinv(randominv, random, consts_p()->ec_order);
    bn_modmul_order(private_key, prx);
    bn_modadd(z, private_key);
    bn_modmul_order(randominv, z);

    //s = random^-1 ⋅ (hash + r * private_key) mod n
    ...
    for (uint32_t i = 0; i < NWORDS; i++)
    {
        for (uint32_t j = 0; j < sizeof(bn_t); j++)
        {
            if (i * sizeof(bn_t) + j >= (BNBYTES - EC_PRIMEBYTES))
            {
                signature[0 * 66 + i * sizeof(bn_t) + j - (BNBYTES - EC_PRIMEBYTES)] = (prx[i] >> (8 * (sizeof(bn_t) - 1 - j))) & 0xFF;
                signature[1 * 66 + i * sizeof(bn_t) + j - (BNBYTES - EC_PRIMEBYTES)] = (randominv[i] >> (8 * (sizeof(
                            bn_t) - 1 - j))) & 0xFF;
            }
        }
    }
