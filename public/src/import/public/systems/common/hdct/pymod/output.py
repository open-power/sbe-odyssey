...
#
# $Source: public/src/import/public/systems/common/hdct/pymod/output.py $
#
...
class output(object):
    ...
    def setupLogging(self, filenameLog, filenameConsole, append=False):
        """
        ...
        if (filenameLog):
            if (filenameLog.endswith(".bz2")):
                mode = "wt" + "+" if append else ""
                self.logFileHandler = logging.StreamHandler(bz2.open(filenameLog, mode=mode, encoding='utf-8'))
            else:
                mode = "a" if append else "w"
                self.logFileHandler = logging.FileHandler(filenameLog, mode=mode)
        ...
        if (filenameConsole):
            if (filenameConsole.endswith(".bz2")):
                mode = "wt" + "+" if append else ""
                self.consoleFileHandler = logging.StreamHandler(bz2.open(filenameConsole, mode=mode, encoding='utf-8'))
            else:
                mode = "a" if append else "w"
                self.consoleFileHandler = logging.FileHandler(filenameConsole, mode=mode)
            # Set the output format
