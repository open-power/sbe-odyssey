...
/*                                                                        */
/* $Source: public/src/runtime/common/core/sbetimersvc.C $                */
/*                                                                        */
...
uint32_t timerService::startTimer(uint32_t i_time, PkTimerCallback i_callBack )
{
    ...
    do
    {
        ...
        if( !iv_init )
        {
            l_pkRc = pk_timer_create(&iv_fixedTimer, i_callBack, NULL);
            if(l_pkRc)
            {
                SBE_ERROR(SBE_FUNC" Pk Timer Create failed, RC=[%d]",
                                    l_pkRc);
        ...
        // Schedule the timer
        l_pkRc = pk_timer_schedule(&iv_fixedTimer,
...
uint32_t timerService::stopTimer( )
{
    ...
    do
    {
        ...
        // Cancel the timer
        int l_pkRc = pk_timer_cancel(&iv_fixedTimer );
        if(l_pkRc)
        {
